  ...
  if (!data?.length) {
    return (
      <div className="space-y-2 text-zinc-600 dark:text-zinc-400">
        <p>
          No rows returned from <code className="text-sm">public.users</code>.
        </p>
        <p className="text-sm">
          If the table has data in Supabase, Row Level Security is likely
          blocking reads. Add a SELECT policy for authenticated users on this
          table.
        </p>
      </div>
    );
        ...
        <div className="mb-6 flex items-center justify-between gap-4">
          <div>
            ...
            <p className="mt-1 text-sm text-zinc-600 dark:text-zinc-400">
              Create and manage Supabase Auth users
            </p>
          </div>
          <Link
            href="/menu"
