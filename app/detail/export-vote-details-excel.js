...
export async function exportVoteDetailsToExcel({
  reportRows,
  buildingName,
  meetingLabel,
  selectedMeetingId,
  ...
  const worksheet = workbook.addWorksheet("Vote Details");
  const columns = getReportColumns(reportRows);

  worksheet.mergeCells(`A1:${MERGE_END_COLUMN}1`);
  const titleCell = worksheet.getCell("A1");
  titleCell.value = "Voting Result Details";
  ...
  reportRows.forEach((row, rowIndex) => {
    ...
    columns.forEach((column, columnIndex) => {
      ...
      cell.value = formatReportCellValue(column.key, row[column.key]);
  ...
  columns.forEach((column, columnIndex) => {
    ...
    const lengths = [
      column.label.length,
      ...reportRows.map((row) =>
        String(formatReportCellValue(column.key, row[column.key])).length
      ),
    ];
    const width = Math.min(Math.max(...lengths, 10) + 2, 40);
    worksheet.getColumn(columnLetter).width =
      columnLetter === "A" ? width * 2 : columnLetter === "B" ? width * 3 : width;
  });
  ...
  link.href = url;
  link.download = `vote-details-meeting-${selectedMeetingId}.xlsx`;
  link.click();
